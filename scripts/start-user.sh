#!/bin/bash
# User mode start script - starts the core program once and waits
set -euo pipefail
...
APP_BIN="${USER_BIN_DIR}/fast_cpp_server"
RUNTIME_LOG_FILE="${USER_LOG_DIR}/fast_cpp_server.log"

# Script logging (for script execution logs)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LOG_DIR="${SCRIPT_DIR}/logs"
TS="$(date '+%Y%m%d_%H%M%S')"
SCRIPT_LOG_FILE="${LOG_DIR}/start_user_${TS}.log"
...
log_line info "Start (user mode) begin: DEBUG=${DEBUG}"
log_line info "Script log: ${SCRIPT_LOG_FILE}"

# Print paths
log_line info "========== Runtime Paths =========="
log_line info "APP_BIN:          ${APP_BIN}"
log_line info "USER_LIB_DIR:     ${USER_LIB_DIR}"
log_line info "USER_CONFIG_DIR:  ${USER_CONFIG_DIR}"
log_line info "USER_LOG_DIR:     ${USER_LOG_DIR}"
...
mkdir -p "${USER_LOG_DIR}" "${USER_RUN_DIR}" "${USER_DATA_DIR}" || true
...
if $DEBUG; then
  ...
  log_line dev "Would redirect to: ${RUNTIME_LOG_FILE}"
